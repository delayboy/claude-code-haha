    ...
    async function* iterator(): AsyncIterator<Item, any, undefined> {
      ...
      try {
        for await (const sse of _iterSSEMessages(response, controller)) {
          ...
          // Detect non-standard SSE errors from 3P providers (e.g., GLM) that
          // omit the 'event: error' SSE line. These arrive as data-only events
          // whose JSON payload is itself an error envelope.
          if (!sse.event) {
            const parsed = safeJSON(sse.data);
            if (parsed && typeof parsed === 'object' && (parsed as Record<string, unknown>).type === 'error') {
              throw new APIError(undefined, parsed, undefined, response.headers);
            }
          }
        }
        done = true;
