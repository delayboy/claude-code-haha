        ...
        async function* iterator() {
            ...
            try {
                for await (const sse of _iterSSEMessages(response, controller)) {
                    ...
                    if (sse.event === 'error') {
                        console.error('[SDK-SSE] standard error event detected, sse.data:', sse.data);
                        throw new error_2.APIError(undefined, (0, values_1.safeJSON)(sse.data) ?? sse.data, undefined, response.headers);
                    }
                    if (!sse.event) {
                        const parsed = (0, values_1.safeJSON)(sse.data);
                        if (parsed && typeof parsed === 'object' && parsed.type === 'error') {
                            console.error('[SDK-SSE] data-only error detected (GLM compat), parsed:', JSON.stringify(parsed));
                            throw new error_2.APIError(undefined, parsed, undefined, response.headers);
                        }
                    }
                    if (sse.data && sse.data.includes('"type":"overloaded_error"')) {
                        console.error('[SDK-SSE] WARNING: overloaded_error in data but NOT detected! sse.event:', sse.event, 'sse.data:', sse.data);
                    }
                }
                done = true;
            }
